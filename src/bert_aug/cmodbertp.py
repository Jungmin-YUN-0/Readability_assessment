...
from transformers import BertTokenizer #수정 전_
#from transformers.tokenization_bert import BertTokenizer 
from transformers.models.bert.modeling_bert import BertForMaskedLM, BertOnlyMLMHead #수정 전_
...
class InputFeatures(object):
    ...
    def __init__(self, init_ids, input_ids, input_mask, labels, label_length):
...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer, seed=12345):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        for index in cand_indexes:
            ...
            labels[index] = tokenizer.convert_tokens_to_ids([tokens[index]])[0]
...
def prepare_data(features):
    ...
    all_labels = torch.tensor([f.labels for f in features],
                                        dtype=torch.long)
    all_label_lengths = torch.tensor([f.label_length for f in features],
                                        dtype=torch.long)
    tensor_data = TensorDataset(all_init_ids, all_input_ids, all_input_mask, all_labels, all_label_lengths)
    return tensor_data
