...
from transformers.models.bert.modeling_bert import BertForSequenceClassification #수정 전_
...
class Classifier:
    def __init__(self, label_list, device, cache_dir):
        ...
        self._model = BertForSequenceClassification.from_pretrained(BERT_MODEL, num_labels=3, cache_dir=cache_dir) #num_labels=len(label_list)
...
def _get_optimizer(model, learning_rate, warmup_steps, t_total):
    ...
    optimizer = torch.optim.Adam(optimizer_grouped_parameters, lr=learning_rate, eps=1e-8)
